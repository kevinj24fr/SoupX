...
validate_soup_channel <- function(sc, require_soup_profile = FALSE, require_rho = FALSE, require_clusters = FALSE) {
  ...
  if (require_soup_profile && is.null(sc$soupProfile)) {
    stop("Soup profile required but not found. Run estimateSoup(sc) first.", call. = FALSE)
  }
...
validate_contamination_fraction <- function(cont_frac, cell_names = NULL) {
  ...
  if (any(cont_frac > 1)) {
    stop("Contamination fraction greater than 1.0 detected (impossible). ",
...
validate_count_matrices <- function(tod, toc) {
  ...
  if (nrow(tod) != nrow(toc)) {
    stop("Gene mismatch: table of droplets (tod) has ", nrow(tod), " genes but table of counts (toc) has ", nrow(toc), " genes. ",
  ...
  if (ncol(toc) > ncol(tod)) {
    stop("Table of counts (toc) cannot have more cells than table of droplets (tod). ",
...
validate_soup_profile <- function(soup_profile, gene_names = NULL) {
  ...
  if (length(missing_cols) > 0) {
    stop("Soup profile missing required columns: ", paste(missing_cols, collapse = ", "), ". ",
