...
adjustCounts <- function(sc,clusters=NULL,method=c('subtraction','soupOnly','multinomial'),roundToInt=FALSE,verbose=1,tol=1e-3,pCut=0.01,...){
  ...
  if(!is.logical(roundToInt)) stop("roundToInt must be logical (TRUE/FALSE,call. = FALSE)",call. = FALSE)
  if(!is.numeric(verbose) || verbose < 0) stop("verbose must be a non-negative integer",call. = FALSE)
  if(!is.numeric(tol) || tol <= 0) stop("tol must be a positive number",call. = FALSE)
  if(!is.numeric(pCut) || pCut <= 0 || pCut >= 1) stop("pCut must be between 0 and 1",call. = FALSE)
  if(is.null(clusters)){
