...
setContaminationFraction <- function(sc,contFrac,forceAccept=FALSE){
  ...
  if(any(contFrac>0.5)){
    stop(sprintf("Extremely high contamination estimated (%.2g%%,call. = FALSE). This likely indicates estimation failure. ",
                 max(contFrac)*100),
...
setDR <- function(sc,DR,reductName=NULL){
  ...
  if(any(is.na(m))){
    ...
    if(nrow(DR)!=nrow(sc$metaData)){
      stop(sprintf("Rownames present in metaData not found in DR and row numbers differ (%d in metaData,%d in DR,call. = FALSE).  Each cell must have a corresponding entry in DR.",nrow(sc$metaData),nrow(DR)))
    }
    m <- seq(nrow(DR))
  }
