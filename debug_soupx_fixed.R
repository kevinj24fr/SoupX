...
# Debug SoupX with proper matrix naming
library(SoupX)
library(Matrix)

cat("=== Debugging SoupX with Proper Matrix Setup ===\n\n")
...
cat("2. Setting proper matrix names...\n")
# Use gene symbols (second column) as row names
gene_names <- genes$V2
rownames(raw_data) <- gene_names
rownames(filtered_data) <- gene_names

# Set column names
colnames(raw_data) <- barcodes_raw$V1
colnames(filtered_data) <- barcodes_filtered$V1

cat("   ✓ Row names set (genes):", length(unique(gene_names)), "unique genes\n")
cat("   ✓ Raw column names set:", ncol(raw_data), "droplets\n")
cat("   ✓ Filtered column names set:", ncol(filtered_data), "cells\n")
cat("   ✓ Sample gene names:", paste(head(gene_names), collapse=", "), "\n")
...
cat("=== Debug Summary ===\n")
cat("✓ Proper matrix naming fixes soup profile calculation\n")
